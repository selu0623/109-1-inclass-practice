...
# NBA1920Final567
NBA1920Final567 <- 
  list(Game5 = list(Data="Oct. 9",Starttime="21:00",Timezone="ET",AWAY="Miami Heat",Home="L.A. Lakers"),
       Game6=list(Data="Oct. 11",Starttime="19:30",Timezone="ET",AWAY="L.A. Lakers",Home="Miami Heat")
       ,Game7=list(Data="Oct. 13",Starttime="21:00",Timezone="ET",AWAY="Miami Heat",Home=" L.A. Lakers"))
```
...
```{r ans12}
# Game5Time
Game5Time <- c(NBA1920Final567$Game5$Data,NBA1920Final567$Game5$Starttime,NBA1920Final567$Game5$Timezone)
```
...
```{r ans13}
# G5TimeParsed
G5TimeParesd <- paste(Game5Time[[1]],2020, Game5Time[[2]])
```
...
```{r ans14}
# TpeJason
ETTime <- mdy_hm(G5TimeParesd,tz="EST")
TpeJason <- with_tz(ETTime,tzone = "Asia/Taipei")
```
...
```{r ans15}
# ParisLouis
ETTimeLouis<- ymd_hms(ETTime+minutes(15))
ParisLouis <- with_tz(ETTimeLouis,tzone="Europe/Paris")
```
...
```{r ans21}
# sumChNumbers
chNumlevels<-c("壹","貳","參","肆","伍","陸","柒","捌","玖")
factorchNumber <- factor(chNumbers,levels=chNumlevels)
numchnum<-as.numeric(factorchNumber)
sumChNumbers<-sum(numchnum)
```
...
```{r ans22}
# sumChNumbersWith0
chNumlevelsplus1<-c("零","壹","貳","參","肆","伍","陸","柒","捌","玖")
factor2 <- factor(chNumbersWith0,levels=chNumlevelsplus1)
numplus1 <- as.numeric(factor2)
sumChNumbersWith0<-sum(numplus1)-randomLength
```
...
```{r ans32}
# dearStudent
dearstudentname<- student$name
dearStudent1<-"```\nDear "
dearStudent1.5<-":\n\n  貼心提醒您，作業上傳截止時間是"
dearStudent2<-list(time=as.POSIXlt(student$deadline))
dearStudent2.5<-as.character(dearStudent2$time)
dearStudent3<-"。\n\n敬祝學安\n\n你的貼心學伴巴西利bot silly\n```"
dearStudent<-paste0(dearStudent1,dearstudentname,dearStudent1.5,dearStudent2.5,dearStudent3)
cat(dearStudent)
```
