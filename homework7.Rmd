...
if(length(str_extract(PaintingName,"(?<=(：))[:graph:]*[唐宋元明清蜀]"))==1)
  return({str_extract(PaintingName,"(?<=(：))[:graph:]*[唐宋元明清蜀]")})
}
...
```
```{r}
get_painting_dynasty(all_name)
length(str_extract(all_name[[4]],"(?<=(：))[:graph:]*[唐宋元明清蜀]"))
```
